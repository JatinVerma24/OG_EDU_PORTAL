...
export default function App() {
  ...
  useEffect(() => {
    const fetchProgrammes = async () => {
      try {
        ...
        const progArray = Array.isArray(json) ? json : (Array.isArray(json?.data) ? json.data : []);
        setProgrammes(progArray);

        // Restore Pinned Programme from localStorage if it exists
        const savedCode = localStorage.getItem('pinned_programme_code');
        if (savedCode) {
          const match = progArray.find(p => p.programmeCode === savedCode);
          if (match) {
            setPinnedProg(match);
          }
        }
  ...
  const progList = Array.isArray(programmes) ? programmes : [];

  // Search Logic with Multi-word Term Expansion and String Normalization
  const filteredProgrammes = progList.filter(prog => {
    // 1. Faculty Filter
  ...
  const uniqueDisciplines = ['All', ...new Set(progList.map(p => p.discipline).filter(Boolean))].sort();
  const uniqueEligibilities = ['All', ...new Set(progList.map(p => p.eligibility).filter(Boolean))].sort();
  const uniqueFaculties = ['All', ...new Set(progList.map(p => p.faculty).filter(Boolean))].sort();
        ...
        <div className="container header-container">
          ...
          <div className="header-actions">
            ...
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer" 
              className="header-cta-link wa-cta animate-pulse"
              title="Join WhatsApp Community"
            >
              <i className="fa-brands fa-whatsapp"></i> <span className="hide-mobile">WhatsApp Community</span>
            </a>
            <a 
              href="https://www.instagram.com/channel/Abb29FGW0tAjvv9F/" 
              target="_blank" 
              rel="noopener noreferrer" 
              className="header-cta-link ig-cta"
              title="Join Instagram Channel"
              style={{ background: 'rgba(236, 72, 153, 0.08)', border: '1px solid rgba(236, 72, 153, 0.2)', color: '#f472b6' }}
            >
              <i className="fa-brands fa-instagram"></i> <span className="hide-mobile">Instagram Channel</span>
            </a>
            <div className="og-badge-wrapper">
              <div className="og-badge">
                <i className="fa-solid fa-crown"></i> DESIGNED BY JATIN VERMA FULL STACK DEVELOPER, CREATOR AND INFLUENCER
              </div>
            </div>
            <button 
      ...
      <main className="container" style={{ position: 'relative', zIndex: 5 }}>
        ...
        <section className="og-resources-section glass-panel">
          ...
          <div className="og-resources-body">
            ...
            <div className="og-links-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
              ...
              <a href="https://chat.whatsapp.com/GTn0Gy3B7yx6Nz0SueAslo" target="_blank" rel="noopener noreferrer" className="og-link-btn wa glow-button">
                <i className="fa-brands fa-whatsapp"></i> Join WhatsApp Community
              </a>
              <a href="https://www.instagram.com/channel/Abb29FGW0tAjvv9F/" target="_blank" rel="noopener noreferrer" className="og-link-btn ig">
